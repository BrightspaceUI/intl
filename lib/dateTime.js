...
function getParts() {
	...
	const result = [];
	const parts = descriptor.formats.dateFormats.short.split(/\W+/).filter(e => e);
...
function convertCldrDatePattern(pattern) {
	let result = '';
	let i = 0;
		...
		if (c === "'") {
			...
			switch (token) {
				...
				case 'mm':
					...
					break;
				default:
					// Unknown time token — pass through as-is
					result += token;
					break;
			}
		} else {
...
function isTimezoneFirst(longFormat) {
	return ['z', 'v'].includes(longFormat.replace(/'.+'/, '').match(/([zv])|([HhaBm])/)[0]);
}
...
export function getDateTimeDescriptor() {
	...
	return settings.getCacheItem('dateTimeDescriptor', () => {
		...
		const timeFormatShort = convertCldrTimePattern(timeSourcePattern);

		// Determine timezone placement from long time format
		const tzFirst = isTimezoneFirst(localeData.timeFormats.long);
		const fullTimeFormat = tzFirst ? `ZZZ ${timeFormatShort}` : `${timeFormatShort} ZZZ`;

		// Convert date formats from CLDR
		const dateFormats = {
			'full': convertCldrDatePattern(localeData.dateFormats.full),
			//'long': convertCldrDatePattern(localeData.dateFormats.long, false),
			'medium': convertCldrDatePattern(localeData.dateFormats.medium),
			'short': convertCldrDatePattern(localeData.dateFormats.short),
			'monthYear': convertCldrDatePattern(localeData.dateFormatItems.yMMMM),
			'monthDay': convertCldrDatePattern(localeData.dateFormatItems.MMMMd),
			'shortMonthDay': convertCldrDatePattern(localeData.dateFormatItems.MMMd),
			'longDayOfWeek': 'cccc',
			'shortDayOfWeek': convertCldrDatePattern(localeData.dateFormatItems.E),
			'longMonth': 'LLLL',
			'shortMonth': convertCldrDatePattern(localeData.dateFormatItems.MMM)
		};
		...
		const descriptor = {
			...
			formats: {
				...
				timeFormats: {
					'full': fullTimeFormat,
					'medium': timeFormatShort, // forced to short to maintain current behavior
					'short': timeFormatShort,
				}
			},
			calendar: {
				...
				monthsStandAlone: {
					abbreviated: localeData.months.standAlone.abbreviated,
					wide: localeData.months.standAlone.wide
				},
				days: {
				...
				daysStandAlone: {
					narrow: localeData.dayNames.standAlone.narrow,
					abbreviated: localeData.dayNames.standAlone.abbreviated,
					wide: localeData.dayNames.standAlone.wide
				},
				dayPeriods: {
		...
		if (settings.overrides.date) {
			if (settings.overrides.date.calendar && settings.language.startsWith(resolveSupportedLocale(settings.language).toLowerCase())) {
				// prefer more nuanced front-end data
...
export function formatDate(date, options) {
	...
	const replacements = {
		...
		// c is unicode
		'cccc': descriptor.calendar.daysStandAlone.wide[date.getDay()],
		'ccc': descriptor.calendar.daysStandAlone.abbreviated[date.getDay()],
		...
		// L is unicode
		'LLLL': descriptor.calendar.monthsStandAlone.wide[date.getMonth()],
		'LLL': descriptor.calendar.monthsStandAlone.abbreviated[date.getMonth()],
...
export function parseDate(input) {
	...
	const dateParts = input.split(/\W+/).filter(e => e);
...
export function formatRelativeDateTime(then) {
	...
	const rtf = new Intl.RelativeTimeFormat(localeData.sourceLocale, {
