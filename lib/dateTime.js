...
export function formatRelativeDateTime(date, { numeric = 'always', style = 'long' } = {}) {

	if (!Intl.RelativeTimeFormat) {
		const now = new Date().toDateString();
		const then = new Date(date).toDateString();

		return now === then ? formatTime(date) : formatDate(date);
	}
	...
	const { value, unit = 'year' } = Object.entries(unitCutoffs).reduce((acc, [unit, cutoff], idx, arr) => {
		if (Math.round(Math.abs(acc.value)) >= Math.round(cutoff)) return { value: acc.value / cutoff };
		arr.length = null; // short-circuit
