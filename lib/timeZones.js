...
export async function getTimeZoneData(
	...
	const locality = identifier.replace(/[^/]*\//, '').replace(/(.*)\/(.*)$/, '$2, $1').replace('_', ' ');
	const region = mapper.getRegionName(identifier) || '';
	const friendlyName = `${region && `${region} - `}${locality.replace(`, ${region}`, '')}`;
	const inputName = `(${offset}) ${localName} - ${locality}`;
