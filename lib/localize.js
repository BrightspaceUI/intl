...
export const getLocalizeClass = (superclass = class {}) => class LocalizeClass extends superclass {
	...
	static async _getLocalizeResources(langs, { importFunc, osloCollection, useBrowserLangs }) {
		...
		const enCA = langs.indexOf('en-ca') < langs.indexOf('en-us'); // en-ca does not have an actual langpack, but supports overrides
		...
		for (const lang of [...langs, fallbackLang]) {
			...
			if (resources) {
				...
				if (osloCollection) {
					...
					let languageId = 0;
					if (oslo.batch) {
						oslo.batch = typeof oslo.batch !== 'string' ? '/d2l/api/oslo/batch' : oslo.batch;
						const batch = new URL(oslo.batch, document.location.href);
					...
					return await getLocalizeOverrideResources(
						lang, // this can potentially be osloLang
						resources,
						() => osloCollection
					);
				}
